...
class Adminpanel_buyleads extends AdminPanel_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('buyleads_model', 'Module_Model');           // MODULE MODEL
        $this->main_tpl = 'adminpanel/buyleads_tpl';                   // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/buyleads_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = MODULE_URL;                          // MODULE URL
        $this->data = $this->Module_Model->general();
        $this->load->helper(array('form', 'url'));
        $this->viewData['buyleadsTab'] = true;
    ...
    function add() {
        ...
        if (!empty($eid)) {
            ...
            $Row_buyleads = $this->Module_Model->Select_buyleads_Rows($eid);
            if (empty($Row_buyleads)) {
                redirect($this->module_url . 'add?' . $this->Module_Model->Appendfk_Country_Site);
            }
            $this->viewData['Row_buyleads'] = $Row_buyleads;
            $edit_record = true;
            if ($Row_buyleads['chrStatus'] == 'L') {
                $edit_record = false;
                if (USERTYPE == 'N' || USERTYPE == 'C' || $this->Module_Model->permissionArry['Approve'] == 'Y') {
                    $edit_record = true;
                }
            }
            $RequirementTypeList = $this->Module_Model->getRequirementTypeList($Row_buyleads['varReqType']);
            $this->viewData['RequirementType'] = $RequirementTypeList;
            $UnitList = $this->Module_Model->getUnitList($Row_buyleads['intUnit']);
            $this->viewData['getUnitData'] = $UnitList;

            $EUnitList = $this->Module_Model->getEUnitList($Row_buyleads['intEUnit']);
            $this->viewData['getEUnitData'] = $EUnitList;

            $Row_customer = $this->Module_Model->CustomerList($Row_buyleads['intUser']);
            $this->viewData['CustomerList'] = $Row_customer;

            $BusinessType = $this->Module_Model->BusinessTypeList($Row_buyleads['varBusinessType']);
            ...
            $BusinessType = $this->Module_Model->BusinessTypeList();
            $this->viewData['getBusinessType'] = $BusinessType;
        }


        $data_page_comb = $this->Module_Model->Bindpageshierarchy('intParentCategory', (!empty($Row_buyleads['intParentCategory']) ? $Row_buyleads['intParentCategory'] : ''), 'add-new-user-textarea2', '1');
        ...
        $this->viewData['buyleadsAddTab'] = true;
...
//        $RequirementTypeList = $this->Module_Model->getRequirementTypeList($var_alias);
    ...
    public function approve_buylead() {
        $this->Module_Model->approve_buylead();
        echo "Y";
    }
...
//        $this->form_validation->set_rules('varLocation', 'Location', 'trim|required');
    ...
    public function load_data() {
        $buyleads_Records = $this->Module_Model->Select_All_buyleads_Record();
        $this->viewData['ShowAllbuyleadsRecords'] = $buyleads_Records;
    ...
    function generate_seocontent() {
        echo $this->Module_Model->generate_seocontent_buyleads(true);
        exit;
    }
    ...
    function set_msg() {
        ...
        $this->session->set_flashdata('msg', $msg_type);
        echo '<a href="' . ADMINPANEL_URL . 'buyleads/get_msg">click</a>';
    }
