...
class services_model extends CI_Model {
    ...
    function SelectAll() {
        ...
        if (!empty($Type)) {
            if ($Type == 'autosearch') {
                ...
                $autoSearchQry = $this->db->query("select *,{$this->SearchByVal} as AutoVal  FROM " . DB_PREFIX . "services where $whereclauseids group by varName order by intDisplayOrder desc");
        ...
        $this->db->from('services as F', false);
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('services');
        return $rs;
    }
    ...
    function Insert() {
//        echo "hi";exit;
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'services WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('services', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
        ...
        if ($_FILES['varImage']['name'] != '') {
            $config['upload_path'] = 'upimages/services/images/';
        ...
        $data = array(
            ...
            'varTagName' => $this->input->post('varTagName', TRUE),
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'services', 'Name' => 'varName', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
    ...
    function update() {
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'services', 'Name' => 'varName', 'ModuleintGlcode' => $int_id, 'Flag' => 'U', 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'services');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }

    function Select_Share_Page_Rows($id) {
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "services WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function get_Post_data($id) {
        $sql = "SELECT p.varName as varName,a.varAlias as varAlias,p.txtDescription as txtDescription,p.varImage as images  FROM " . DB_PREFIX . "services as p
        ...
        $image = SITE_PATH . 'upimages/services/' . $result['images'];
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'services', 'Name' => 'varName', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id');
            $this->mylibrary->insertinlogmanager($ParaArray);
        }
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'services');
    }
    ...
    function updatedisplayorder() {
        ...
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'services');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'services');
    }

    function get_hits($id) {
        $this->db->cache_set_common_path("application/cache/db/common/services/");
    ...
    function SelectAll_front() {
        $sql = "select * from " . DB_PREFIX . "services WHERE chrPublish='Y' and chrDelete='N' group by int_id order by intDisplayOrder desc";
