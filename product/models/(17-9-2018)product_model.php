...
class product_model extends CI_Model {
    ...
    var $PageSize = 1; // Attribute of Org_calendarize For Paging
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('product');
        return $rs;
    }
...
//        $Result = $query->result_array();
    ...
    public function SelectAll_Detail_front($id) {
        $sql = "select p.*,A.varAlias,au.varAlias as usersite,u.varCompany as CompanyName,ci.varOwnerType,ci.varTotalEmployees,ci.varRegistration,u.varCity,u.varCountry,ci.varBusinessType,um.varName as PriceUnit,uum.varName as PUnitName,umq.varName as MOQUnit "
...
//        $project = $this->DataModel->getbyid('productgallery', array('fkProduct' => '1'));
    ...
    public function getCategoryProductsData($id = '') {
        $sql = "select E.*,A.varAlias,AU.varAlias as usersite,U.varCity,U.varCountry,U.varCompany,U.varSubdomain from " . DB_PREFIX . "product E "
                ...
                . "WHERE U.chrType='BS' and E.chrPublish='Y' and E.intParentCategory='" . $id . "' and A.fk_ModuleGlCode='140' and AU.fk_ModuleGlCode='136' and E.chrDelete='N' group by E.int_id order by E.intDisplayOrder asc limit 20";
//        echo $sql;exit;
        $query = $this->db->query($sql);
        $data = $query->result_array();
        return $data;
...
//        $type = $this->input->get_post('type');
        ...
        $data = array();
        $sql = "";
        $sql = "select p.*,A.varAlias,u.varCompany as CompanyName,u.varSubdomain,u.varCity,u.varCountry,ci.varBusinessType,um.varName as PriceUnit,umq.varName as MOQUnit "
                . "from " . DB_PREFIX . "product p "
                . "LEFT JOIN " . DB_PREFIX . "alias as A ON p.int_id=A.fk_Record "
                . "LEFT JOIN " . DB_PREFIX . "users as u ON p.intSupplier=u.int_id "
                . "LEFT JOIN " . DB_PREFIX . "company_information as ci ON ci.int_id=p.intSupplier "
...
//       echo $sql;exit;
    ...
    function getCategoryNames() {
        ...
        $sql = "select pc.varName as CategoryName,pc.int_id "
                ...
                . "order by CASE WHEN p.varName LIKE '$keyword%' THEN 1 WHEN p.varName LIKE '%$keyword' THEN 3 ELSE 2 END, p.varName asc limit 10";
//       echo $sql;exit;
        $query = $this->db->query($sql);
        $data = $query->result_array();
...
//        echo "</pre>";
        ...
        foreach ($perentcat as $cats) {
            ...
            $getSubParentCategoryData = $this->getSubCatData($cats['intPerentCategoryId']);
            if ($i >= '3') {
...
//        $type = $this->input->get_post('type');
        ...
        $data = array();
        $sql = "";
        $sql = "select p.*,A.varAlias,u.varCompany as CompanyName,u.varCity,u.varCountry,ci.varBusinessType,um.varName as PriceUnit,umq.varName as MOQUnit "
                . "from " . DB_PREFIX . "product p "
                . "LEFT JOIN " . DB_PREFIX . "alias as A ON p.int_id=A.fk_Record "
                . "LEFT JOIN " . DB_PREFIX . "users as u ON p.intSupplier=u.int_id "
                . "LEFT JOIN " . DB_PREFIX . "company_information as ci ON ci.int_id=p.intSupplier "
...
//        }
    ...
    public function getProductSearch($id) {
        $type = $this->input->get_post('type');
        $keywords = $this->input->get_post('keyword');
...
//        $this->db->where_not_in('int_id', $NotShowString);
    ...
    function getBusinessType($dat = '') {
        ...
        $data = explode(",", $dat);
        foreach ($data as $id) {
            $sql = "select varName FROM " . DB_PREFIX . "designation where chrDelete='N' AND int_id='" . $id . "'";
            $rs = $this->db->query($sql);
            $row = $rs->row_array();
            $html .= $row['varName'] . ", ";
        }
    ...
    function getSupplierList($id = '') {
        $query = $this->db->query("select * from " . DB_PREFIX . "users where chrDelete = 'N' and chrPublish='Y' and chrPayment='Y' order by intPlan desc,RAND() limit 4");
        $Result = $query->result_array();
        return $Result;
    }
