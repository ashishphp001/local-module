...
class careers extends Front_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('careers_model', 'Module_Model');
        $this->module_url = FRONT_MODULE_URL;
        $this->main_tpl = 'front/careers_tpl';
    }
    ...
    public function index() {
        ...
        if ($this->input->get_post('ajax', TRUE) == 'Y') {
            ...
            $this->viewData['ContentPanel'] = 'front/careers_tpl';
        }
        $this->load_view();
    }
...
//        echo "here";exit;
        $CurrentPageData = $this->common_model->getPageData_Seo(RECORD_ID, "careers");
        ...
        $Seo_array['description'] = $CurrentPageData['varMetaDescription'];
        $this->common_model->get_metadata($Seo_array);
        $this->viewData['ContentPanel'] = 'front/careers_detail_tpl';
