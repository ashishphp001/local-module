...
class Adminpanel_careers extends AdminPanel_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('careers_model', 'Module_Model');           // MODULE MODEL
        $this->main_tpl = 'adminpanel/careers_tpl';                   // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/careers_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = MODULE_URL;                          // MODULE URL
        $this->data = $this->Module_Model->general();
        $this->load->helper(array('form', 'url'));
        $this->viewData['careersTab'] = true;
    ...
    function add() {
        ...
        if (!empty($eid)) {
            ...
            $Row_careers = $this->Module_Model->Select_careers_Rows($eid);
            if (empty($Row_careers)) {
                redirect($this->module_url . 'add?' . $this->Module_Model->Appendfk_Country_Site);
            }
            $this->viewData['Row_careers'] = $Row_careers;
            $edit_record = true;
            if ($Row_careers['chrStatus'] == 'L') {
                $edit_record = false;
        ...
        $this->viewData['careersAddTab'] = true;
    ...
    public function load_data() {
        $careers_Records = $this->Module_Model->Select_All_careers_Record();
        $this->viewData['ShowAllcareersRecords'] = $careers_Records;
    ...
    function generate_seocontent() {
        echo $this->Module_Model->generate_seocontent_careers(true);
        exit;
    }
    ...
    function set_msg() {
        ...
        $this->session->set_flashdata('msg', $msg_type);
        echo '<a href="' . ADMINPANEL_URL . 'careers/get_msg">click</a>';
    }
