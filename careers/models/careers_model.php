...
class careers_model extends CI_Model {
    ...
    var $numofcareers; // Attribute of Num Of Pagues In Result
    var $OrderBy = 't.intDisplayOrder'; // Attribute of Deafult Order By
    ...
    function Select_All_careers_Record() {
        $this->initialize();
        $this->Generateurl();
        ...
        if (!empty($Type)) {
            if ($Type == 'autosearch') {
                ...
                $autoSearchQry = $this->db->query("select *,{$this->SearchByVal} as AutoVal  FROM " . DB_PREFIX . "careers t where  $whereclauseids group by t.varTitle $OrderBy");
...
//        $this->db->select('a.varAlias,a.int_id AS alias_id,a.intPageHits,a.intMobileHits', false);
        ...
        $this->db->from('careers AS t', false);
//        $this->db->join('alias AS a', 'a.fk_Record = t.int_id', 'left', false);
    ...
    function Insert($Images_Name = '') {
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'careers WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        ...
        $meta_data = $this->generate_seocontent_careers();
        ...
        if ($_FILES['varImage']['name'] != '') {
            $config['upload_path'] = 'upimages/careers/images/';
        ...
        if ($_FILES['varImage1']['name'] != '') {
            ...
            $Imagesurl1 = $this->ImageName1 = $this->common_model->Clean_String($this->ImageName1);
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('careers', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
            $Int_DisplayOrder = $this->input->post('intDisplayOrder');
        } else {
            $Int_DisplayOrder = $tot_recods + 1;
        }

        $dtCareerDate1 = $this->input->post('dtCareerDate');
        $dtCareerDate = date("Y-m-d", strtotime($dtCareerDate1));
        ...
        $data = array(
            ...
            'varPositionName' => $this->input->post('varPositionName', TRUE),
            'varCity' => $this->input->post('varCity', TRUE),
            'varTime' => $this->input->post('varTime', TRUE),
            'chrImageFlag' => 'S',
            'chrImageFlag1' => 'S',
            'varImage' => $Imagesurl,
            'varImage1' => $Imagesurl1,
            'dtCareerDate' => $dtCareerDate,
...
//        print_r($data);
//        exit;
        $query = $this->db->insert(DB_PREFIX . 'careers', $data);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'careers', 'Name' => 'varTitle', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
    ...
    function update() {
        ...
        $this->db->update(DB_PREFIX . 'careers', $data);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'careers', 'Name' => 'varTitle', 'ModuleintGlcode' => $int_id, 'Flag' => $opertion, 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'careers');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }
    ...
    function updatedisplayorder() {
        ...
        $fkcareers = $this->input->get_post('fkcareers');
        if (empty($fkcareers)) {
            $fkcareers = 0;
        }
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'careers', "");
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'careers');
    }
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'careers', 'Name' => 'varTitle', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id', 'fk_Country' => $this->fk_Country, 'fk_Site' => $this->fk_Website);
            $this->mylibrary->insertinlogmanager($ParaArray);
            $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'careers');
        }
    }
...
//        $varFile = $this->input->get_post('varFile', true);
        ...
        $uploads_dir = 'upimages/careerleads';
        move_uploaded_file($tmp_name, $uploads_dir . "/" . $Filesurl);


        $intCareer = $this->input->get_post('intCareer', true);
...
//        $this->send_contact_info();
    ...
    function generate_seocontent_careers($fromajax = false) {
    ...
    function Select_Share_Page_Rows($id) {
        ...
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "careers WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function get_Post_data($id) {
        ...
        $sql = "SELECT p.varTitle as varTitle,a.varAlias as varAlias,p.txtDescription as txtDescription,p.varImage as varImage  FROM " . DB_PREFIX . "careers as p
        ...
        $image = SITE_PATH . 'upimages/careers/' . $result['varImage'];
    ...
    function get_publish_value($id) {
        $returnArry = array();
        $sql = "SELECT chrPublish FROM " . DB_PREFIX . "careers WHERE chrDelete='N' AND int_id='" . $id . "'";
        $query = $this->db->query($sql);
        $result = $query->row_array();
...
//        echo "select pc.int_id AS id, pc.varTitle AS name from " . DB_PREFIX . "product as p left join " . DB_PREFIX . "productcategory as pc on pc.int_id = p.intSharePointCategory  where  p.chrDelete = 'N' group by pc.varTitle order by pc.intDisplayOrder asc";
        $query = $this->db->query("select pc.int_id AS id, pc.varTitle AS name from " . DB_PREFIX . "careers as p left join " . DB_PREFIX . "SharePointCategory as pc on pc.int_id = p.intSharePointCategory  where  p.chrDelete = 'N' group by pc.varTitle order by pc.intDisplayOrder asc");
        $returnHtml = "";
...
//            print_r($row);
    ...
    public function getPositionname() {
        $sql = "select * from " . DB_PREFIX . "careers WHERE chrPublish='Y' and chrDelete='N' order by intDisplayOrder asc";
        $query = $this->db->query($sql);
    ...
    public function SelectAll_front() {
        ...
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "careers as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record WHERE  A.fk_ModuleGlCode='99'  and N.chrPublish='Y' and N.chrDelete='N' group by N.int_id order by N.intDisplayOrder desc,N.int_id desc $limitby";
        $query = $this->db->query($sql);
        $data = $query->result_array();
        return $data;
    }
    ...
    public function SelectAll_detail_front_id($id) {
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "careers as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record WHERE  A.fk_ModuleGlCode='124'  and N.chrPublish='Y' and N.chrDelete='N' and N.int_id!='" . $id . "' group by N.int_id order by N.intDisplayOrder desc,N.int_id desc";
        $query = $this->db->query($sql);
        $data = $query->result_array();
        return $data;
    }
    ...
    function CountRow_front() {
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "careers as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record AND A.fk_ModuleGlCode='96' WHERE N.chrPublish='Y' and N.chrDelete='N' group by N.int_id order by N.intDisplayOrder asc ";
        $query = $this->db->query($sql);
    ...
    public function SelectAll_Detail_front($id) {
        $sql = " select * from " . DB_PREFIX . "careers where chrDelete='N' and int_id='" . $id . "'";
        $query = $this->db->query($sql);
