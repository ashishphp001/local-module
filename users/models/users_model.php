...
class users_model extends CI_Model {
    ...
    function generateParam($position = 'top') {
        ...
        return array(
            ...
            'search' => array('searchArray' => array("varCompany" => "Title"),
    ...
    function SelectAll() {
        ...
        if ($this->SearchTxt != '') {
            $whereclauseids .= (empty($this->SearchBy)) ? " AND varCompany like '%" . addslashes($this->SearchTxt) . "%'" : " AND $this->SearchBy like '%" . addslashes($this->SearchTxt) . "%'";
        }
        if ($this->FilterBy != '0') {
        ...
        $whereclause .= (empty($this->SearchBy)) ? " AND P.varCompany like '%" . addslashes($this->SearchTxt) . "%'" : " AND P.$this->SearchBy like '%" . addslashes($this->SearchTxt) . "%' AND P.chrDelete='N'";
    ...
    function Insert($Files_Name = '') {
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('users', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
...
//echo'Team';exit;
        $sql = "select * from " . DB_PREFIX . "users WHERE chrPublish='Y' and chrDelete='N' and intDisplayOrder<= CURDATE() group by int_id order by intDisplayOrder desc $limitby ";
    ...
    function CountServices() {
        ...
        $sql = "select * from " . DB_PREFIX . "users WHERE chrPublish='Y' and chrDelete='N' group by int_id order by intDisplayOrder desc ";
        $query = $this->db->query($sql);
