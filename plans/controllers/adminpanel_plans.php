...
class Adminpanel_plans extends AdminPanel_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('plans_model', 'Module_Model');           // MODULE MODEL
        $this->main_tpl = 'adminpanel/plans_tpl';                   // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/plans_add_tpl';              // MODULE ADD  VIEW
        $this->plans_feature_tpl = 'adminpanel/plans_feature_tpl';              // MODULE ADD  VIEW
        $this->module_url = MODULE_URL;                          // MODULE URL
        $this->data = $this->Module_Model->general();
        $this->load->helper(array('form', 'url'));
        $this->viewData['plansTab'] = true;
    ...
    function feature() {
        ...
        if (!empty($eid)) {
            ...
            $Row_plans = $this->Module_Model->Select_plans_Rows($eid);

            if (empty($Row_plans)) {
                redirect($this->module_url . 'add');
            }
            $this->viewData['Row_plans'] = $Row_plans;
            $getFeaturesList = $this->Module_Model->getFeaturesList();
            $this->viewData['getFeaturesList'] = $getFeaturesList;

            $action = $this->module_url . 'update_feature?' . $_SERVER['QUERY_STRING'];
        } else {
        ...
        $this->viewData['plansAddTab'] = true;
        $this->viewData['adminContentPanel'] = $this->plans_feature_tpl;
        $this->load_view();
    }
    ...
    public function update_feature() {
        $this->Module_Model->update_features();
        $this->Redirect_feature_page($this->input->get_post('ehintglcode'), 'edit');
    }

    public function Redirect_feature_page($id, $msg_type) {
        ...
        if (!empty($btnsaveandc_x)) {
            redirect($this->module_url . 'feature?eid=' . $id);
        } else {
    ...
    public function load_data() {
        $plans_Records = $this->Module_Model->Select_All_plans_Record();
        $this->viewData['ShowAllplansRecords'] = $plans_Records;
    ...
    function generate_seocontent() {
        echo $this->Module_Model->generate_seocontent_plans(true);
        exit;
    }
    ...
    function set_msg() {
        ...
        $this->session->set_flashdata('msg', $msg_type);
        echo '<a href="' . ADMINPANEL_URL . 'plans/get_msg">click</a>';
    }
