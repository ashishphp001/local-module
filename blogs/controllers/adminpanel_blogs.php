...
class Adminpanel_blogs extends AdminPanel_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('blogs_model', 'Module_Model');           // MODULE MODEL
        $this->main_tpl = 'adminpanel/blogs_tpl';                   // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/blogs_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = MODULE_URL;                          // MODULE URL
        $this->data = $this->Module_Model->general();
        $this->load->helper(array('form', 'url'));
        $this->viewData['blogsTab'] = true;
    ...
    function add() {
        ...
        if (!empty($eid)) {
            ...
            $Row_blogs = $this->Module_Model->Select_blogs_Rows($eid);
            if (empty($Row_blogs)) {
                redirect($this->module_url . 'add?' . $this->Module_Model->Appendfk_Country_Site);
            }
            $this->viewData['Row_blogs'] = $Row_blogs;
            $edit_record = true;
            if ($Row_blogs['chrStatus'] == 'L') {
                $edit_record = false;
        ...
        $this->viewData['blogsAddTab'] = true;
    ...
    public function load_data() {
        $blogs_Records = $this->Module_Model->Select_All_blogs_Record();
        $this->viewData['ShowAllblogsRecords'] = $blogs_Records;
    ...
    function generate_seocontent() {
        echo $this->Module_Model->generate_seocontent_blogs(true);
        exit;
    }
    ...
    function set_msg() {
        ...
        $this->session->set_flashdata('msg', $msg_type);
        echo '<a href="' . ADMINPANEL_URL . 'blogs/get_msg">click</a>';
    }
