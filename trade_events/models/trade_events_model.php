...
class trade_events_model extends CI_Model {
    ...
    var $numoftrade_events; // Attribute of Num Of Pagues In Result
    ...
    function generateParam($position = 'top') {
        ...
        return array(
            ...
            'tablename' => DB_PREFIX . 'trade_events',
            'position' => $position,
    ...
    function Select_All_trade_events_Record() {
        $this->initialize();
        $this->Generateurl();
        ...
        if (!empty($Type)) {
            if ($Type == 'autosearch') {
                ...
                $autoSearchQry = $this->db->query("select *,{$this->SearchByVal} as AutoVal  FROM " . DB_PREFIX . "trade_events t where  $whereclauseids group by t.varTitle $OrderBy");
...
//        $this->db->select('a.varAlias,a.int_id AS alias_id,a.intPageHits,a.intMobileHits', false);
        ...
        $this->db->from('trade_events AS t', false);
//        $this->db->join('alias AS a', 'a.fk_Record = t.int_id', 'left', false);
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('trade_events');
        return $rs;
    }

    function Select_trade_events_Rows($id) {
        $returnArry = array();
        $wherecondtion = array('T.chrDelete' => 'N', 'T.int_id' => $id);
        $this->db->select('T.*,a.varAlias,a.int_id as Alias_Id');
        $this->db->from('trade_events As T');
    ...
    function Insert($Images_Name = '') {
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'trade_events WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        ...
        $meta_data = $this->generate_seocontent_trade_events();
        ...
        if ($_FILES['varImage']['name'] != '') {
            $config['upload_path'] = 'upimages/trade_events/images/';
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('trade_events', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
            $Int_DisplayOrder = $this->input->post('intDisplayOrder');
        } else {
            $Int_DisplayOrder = $tot_recods + 1;
        }

        $dtEventDate1 = $this->input->post('dtEventDate');
        $dtEventDate = date("Y-m-d", strtotime($dtEventDate1));
...
//        print_r($data);
//        exit;
        $query = $this->db->insert(DB_PREFIX . 'trade_events', $data);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'trade_events', 'Name' => 'varTitle', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
    ...
    function update() {
        ...
        $dtEventDate = date("Y-m-d", strtotime($dtEventDate1));
        if (date('Y', strtotime($dtEventDate)) == '1970') {
              $dtEventDates1 = $this->input->post('dtEventDate1');
            $dtEventDate = date("Y-m-d", strtotime($dtEventDates1));
        }
//  echo $dtEventDate;exit;
...
//        exit;
        ...
        $this->db->update(DB_PREFIX . 'trade_events', $data);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'trade_events', 'Name' => 'varTitle', 'ModuleintGlcode' => $int_id, 'Flag' => $opertion, 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'trade_events');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }
    ...
    function updatedisplayorder() {
        ...
        $fktrade_events = $this->input->get_post('fktrade_events');
        if (empty($fktrade_events)) {
            $fktrade_events = 0;
        }
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'trade_events', "");
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'trade_events');
    }
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'trade_events', 'Name' => 'varTitle', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id', 'fk_Country' => $this->fk_Country, 'fk_Site' => $this->fk_Website);
            $this->mylibrary->insertinlogmanager($ParaArray);
            $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'trade_events');
        }
    }
    ...
    function generate_seocontent_trade_events($fromajax = false) {
    ...
    function Select_Share_Page_Rows($id) {
        ...
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "trade_events WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function get_Post_data($id) {
        ...
        $sql = "SELECT p.varTitle as varTitle,a.varAlias as varAlias,p.txtDescription as txtDescription,p.varImage as varImage  FROM " . DB_PREFIX . "trade_events as p
        ...
        $image = SITE_PATH . 'upimages/trade_events/' . $result['varImage'];
    ...
    function get_publish_value($id) {
        $returnArry = array();
        $sql = "SELECT chrPublish FROM " . DB_PREFIX . "trade_events WHERE chrDelete='N' AND int_id='" . $id . "'";
        $query = $this->db->query($sql);
        $result = $query->row_array();
...
//        echo "select pc.int_id AS id, pc.varTitle AS name from " . DB_PREFIX . "product as p left join " . DB_PREFIX . "productcategory as pc on pc.int_id = p.intSharePointCategory  where  p.chrDelete = 'N' group by pc.varTitle order by pc.intDisplayOrder asc";
        $query = $this->db->query("select pc.int_id AS id, pc.varTitle AS name from " . DB_PREFIX . "trade_events as p left join " . DB_PREFIX . "SharePointCategory as pc on pc.int_id = p.intSharePointCategory  where  p.chrDelete = 'N' group by pc.varTitle order by pc.intDisplayOrder asc");
        $returnHtml = "";
...
//            print_r($row);
    ...
    public function SelectAll_front() {
        ...
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "trade_events as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record WHERE  A.fk_ModuleGlCode='124'  and N.chrPublish='Y' and N.chrDelete='N' group by N.int_id order by N.intDisplayOrder desc,N.int_id desc $limitby";
        $query = $this->db->query($sql);
    ...
    public function SelectAll_detail_front_id($id) {
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "trade_events as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record WHERE  A.fk_ModuleGlCode='124'  and N.chrPublish='Y' and N.chrDelete='N' and N.int_id!='" . $id . "' group by N.int_id order by N.intDisplayOrder desc,N.int_id desc";
        $query = $this->db->query($sql);
        $data = $query->result_array();
        return $data;
    }
    ...
    function CountRow_front() {
        $sql = "select N.*,A.varAlias from " . DB_PREFIX . "trade_events as N LEFT JOIN " . DB_PREFIX . "alias as A ON N.int_id=A.fk_Record AND A.fk_ModuleGlCode='96' WHERE N.chrPublish='Y' and N.chrDelete='N' group by N.int_id order by N.intDisplayOrder asc ";
        $query = $this->db->query($sql);
    ...
    public function SelectAll_Detail_front($id) {
        $sql = " select * from " . DB_PREFIX . "trade_events where chrDelete='N' and int_id='" . $id . "'";
        $query = $this->db->query($sql);
        $data = $query->row_array();
        return $data;
    }
