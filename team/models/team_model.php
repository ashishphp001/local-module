...
class team_model extends CI_Model {
    ...
    function SelectAll() {
        ...
        if (!empty($Type)) {
            if ($Type == 'autosearch') {
                ...
                $autoSearchQry = $this->db->query("select *,{$this->SearchByVal} as AutoVal  FROM " . DB_PREFIX . "team where $whereclauseids group by varName order by intDisplayOrder desc");
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('team');
        return $rs;
    }
    ...
    function Insert($Files_Name = '') {
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'team WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        $publish = $this->input->post('chrPublish', true);
        $chrPublish = ($publish != '') ? $publish : 'Y';
        if ($_FILES['varImage']['name'] != '') {
            $config['upload_path'] = 'upimages/team/images/';
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('team', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'team', 'Name' => 'varName', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
    ...
    function update($Files_Name = '') {
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'team', 'Name' => 'varName', 'ModuleintGlcode' => $int_id, 'Flag' => 'U', 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'team');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }
...
//echo "Select_Share_Page_Rows";exit;
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "team WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function get_Post_data($id) {
        $sql = "SELECT p.varName as varName,a.varAlias as varAlias,p.txtDescription as txtDescription,p.varImage as images  FROM " . DB_PREFIX . "team as p
        ...
        $image = SITE_PATH . 'upimages/team/' . $result['images'];
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'team', 'Name' => 'varName', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id');
            $this->mylibrary->insertinlogmanager($ParaArray);
        }
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'team');
    }
    ...
    function updatedisplayorder() {
        ...
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'team');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'team');
    }

    function get_hits($id) {
        $this->db->cache_set_common_path("application/cache/db/common/team/");
...
//echo'Team';exit;
        $sql = "select * from " . DB_PREFIX . "team WHERE chrPublish='Y' and chrDelete='N' and intDisplayOrder<= CURDATE() group by int_id order by intDisplayOrder desc $limitby ";
    ...
    function CountServices() {
        ...
        $sql = "select * from " . DB_PREFIX . "team WHERE chrPublish='Y' and chrDelete='N' group by int_id order by intDisplayOrder desc ";
        $query = $this->db->query($sql);
