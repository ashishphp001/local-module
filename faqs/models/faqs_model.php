    ...
    function SelectAll() {
        ...
        if (!empty($Type)) {
            if ($Type == 'autosearch') {
                ...
                $autoSearchQry = $this->db->query("select *,{$this->SearchByVal} as AutoVal  FROM " . DB_PREFIX . "faqs where $whereclauseids group by varName order by intDisplayOrder desc");
...
//        }
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('faqs');
        return $rs;
    }
    ...
    function Insert($Files_Name = '') {
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'faqs WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('faqs', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'faqs', 'Name' => 'varName', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
    ...
    function update($Files_Name = '') {
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'faqs', 'Name' => 'varName', 'ModuleintGlcode' => $int_id, 'Flag' => 'U', 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'faqs');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }

    function Select_Share_Page_Rows($id) {
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "faqs WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'faqs', 'Name' => 'varName', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id');
            $this->mylibrary->insertinlogmanager($ParaArray);
        }
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'faqs');
    }
    ...
    function updatedisplayorder() {
        ...
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'faqs');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'faqs');
    }

    function get_hits($id) {
        $this->db->cache_set_common_path("application/cache/db/common/faqs/");
    ...
    function SelectAll_front() {
        $sql = "select * from " . DB_PREFIX . "faqs WHERE chrPublish='Y' and chrDelete='N' group by int_id order by intDisplayOrder desc";
