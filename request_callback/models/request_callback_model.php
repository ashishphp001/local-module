...
class request_callback_model extends CI_Model {
    ...
    var $OrderBy = 'F.int_id'; // Attribute of Deafult Order By
    ...
    function initialize($flag = '') {
        ...
        if ($flag == 'Y') {
            $this->NumOfRows = $this->Countrequest_callback();
    ...
    function generateParam($position = 'top') {
        ...
        return array(
            ...
            'heading' => "Manage Request Call Back",
            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'request_callback',
            'position' => $position,
    ...
    function SelectAll() {
        ...
        $this->Generateurl();
        $whereclauseids = '';
        $whereclauseids = " F.chrDelete ='N' ";
        $this->db->select('F.*,u.varName,u.varEmail,u.varPhone,u.varTel,u.varCompany', false);
        $this->db->from('request_callback as F', false);
        $this->db->join('users AS u', 'u.int_id = F.intUser', 'left', false);
...
//        }
    ...
    function CountRows() {
        ...
        $rs = $this->db->count_all_results('request_callback');
        return $rs;
    }
    ...
    function Select_Rows($id) {
        ...
        $this->db->from('request_callback As F');
    ...
    function Insert($Files_Name = '') {
        $query = $this->db->query('SELECT count(1) as total FROM ' . DB_PREFIX . 'request_callback WHERE chrDelete = "N"');
        $rs = $query->row();
        $tot_recods = $rs->total;
        ...
        if ($tot_recods >= $this->input->post('intDisplayOrder')) {
            $this->mylibrary->UpdateInt_DisplayOrder('request_callback', $this->input->post('intDisplayOrder'), $DisplayOrderClause);
        ...
        $this->db->insert('request_callback', $data);
        $id = $this->db->insert_id();
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'request_callback', 'Name' => 'varName', 'ModuleintGlcode' => $id, 'Flag' => 'I', 'Default' => 'int_id');
        $this->mylibrary->insertinlogmanager($ParaArray);
        return $id;
    }

    function updateStatus() {
        $status = $this->input->post('chrStatus', true);
        ...
        $this->db->update('request_callback', $data);
    }
    ...
    function update() {
        ...
        $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'request_callback', 'Name' => 'varName', 'ModuleintGlcode' => $int_id, 'Flag' => 'U', 'Default' => 'int_id');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'request_callback');
        $this->mylibrary->insertinlogmanager($ParaArray);
    }

    function Select_Share_Page_Rows($id) {
        $returnArry = array();
        $sql = "SELECT * FROM " . DB_PREFIX . "request_callback WHERE chrDelete='N' AND chrPublish='Y' AND int_id='" . $id . "'";
...
//            print_r($param);exit;
    ...
    function delete_row() {
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            ...
            $ParaArray = array('ModelId' => MODULE_ID, 'TableName' => DB_PREFIX . 'request_callback', 'Name' => 'varName', 'ModuleintGlcode' => $deletearray[$i], 'Flag' => 'D', 'Default' => 'int_id');
            $this->mylibrary->insertinlogmanager($ParaArray);
        }
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'request_callback');
    }
    ...
    function updatedisplayorder() {
        ...
        $this->mylibrary->update_display_order_Ajax($uids, $neworder, $oldorder, '', 'request_callback');
        $this->mylibrary->set_Int_DisplayOrder_sequence(DB_PREFIX . 'request_callback');
    }

    function get_hits($id) {
        $this->db->cache_set_common_path("application/cache/db/common/request_callback/");
    ...
    function SelectAll_front() {
        $sql = "select * from " . DB_PREFIX . "request_callback WHERE chrPublish='Y' and chrDelete='N' group by int_id order by intDisplayOrder desc";
    ...
    function checkdata() {
        $sql = "select r.*,u.varName from " . DB_PREFIX . "request_callback as r left join " . DB_PREFIX . "users as u on u.int_id=r.intUser WHERE r.chrStatus='W' and r.chrDelete='N' and r.dtCreateDate > ('" . date('Y-m-d H:i:s') . "' - interval 30 minute)";
        $data = $this->db->query($sql);
        $result_query = $data->result_array();
        return $result_query;
    }
    ...
    function Countrequest_callback() {
